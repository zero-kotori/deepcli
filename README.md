...
`recipes sota --json` 也会在顶层输出非阻塞 `recommendedOpportunity`、`opportunityPriorityCounts` 和 `opportunities[]`，复用当前 `round` 的机会对象，让产品循环入口除了按钮队列外还能展示每个机会的 summary、impact、priority、effort、status 和 checklist，并给外部 UI 一个无需扫描数组的主推荐；文本模式也会在 opportunities 列表前展示推荐机会和优先级计数，方便终端用户直接判断先做什么。

`opportunities --json` 是这些非阻塞机会的一等只读入口，输出稳定 `deepcli.opportunities.v1`，直接复用当前 `round` 的机会对象并给出顶层 `filter`、`recommendedOpportunity`、`opportunityPriorityCounts`、`availablePriorityCounts`、`nextActions` 与 `checklist[]`；`--priority high|medium|low|other` 会把机会列表、推荐机会和动作队列收敛到指定优先级，同时保留总机会数、过滤掉的数量和全量优先级计数；文本模式同样展示 `recommended opportunity` 和 `priority counts` 摘要；每个机会都包含 `impact`、`priority` 与 `effort`，让用户或外部 UI 能同时展示收益、优先级和执行成本；ready 的 `scorecard`、`round` 和 `recipes sota` 顶层动作也会暴露 `deepcli opportunities --json`，让用户或外部 UI 不必解析完整报告即可打开机会页；当 benchmark evidence 已 ready 但 freshness 为 aging 时，机会页会把刷新 benchmark evidence 作为高优先级低成本机会放在前面；baseline 机会会先给 `deepcli benchmark baselines --json` 作为只读 inventory 检查，再给写入或 compare 动作；当当前 round 还没有可展示机会时，下一步动作会回退为 round 修复队列。
