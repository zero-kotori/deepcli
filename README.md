...
`next --json` / `session next --json` 面向恢复 UI 和脚本输出可执行的 `deepcli ...` 动作队列，并从 `nextActions` 派生顶层 `checklist[]`；`session diagnose --json` 的 `recommendedNextActions` 和 `quickLinks` 使用同一格式，顶层 `checklist[]` 来自推荐动作，说明性上下文保留在 `signals` 与 `report`。
