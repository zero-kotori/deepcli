...
`quickstart --json`、`selftest --json`、`version/about --json`、`health/doctor --json`、`diagnose/support --json` 和 support bundle `manifest.json` 会从可执行 `nextActions` 派生顶层 `checklist[]`，每项包含 `step`、`label` 和 `command`，让 TUI、外部 onboarding UI、健康面板、支持面板、安装脚本或验收脚本可以直接渲染下一步动作；首次引导和诊断说明继续放在 `steps`、`report`、`environment`、`shell`、`notes` 等解释性字段中，外部 UI 或脚本不需要解析 `run \`/...\`` 文本才能推进下一步。
