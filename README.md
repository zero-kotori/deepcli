...
`model show/list --json`、`timeout --json`、`logs --json`、`prompt list|get|render --json`、`skill list|run --json` 和 `agent list|show --json` 也遵守同一原则：顶层 `nextActions` 只输出可直接复制执行的 `deepcli ...` 命令；当前已有 prompt、skill 或 agent 任务时优先给出具体名称或短 id，说明性上下文保留在 `report` 或条目字段。
