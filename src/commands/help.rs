    ...
    &[
        ...
        CommandHelp {
            ...
            notes: &[
                ...
                "Start in any project directory with `deepcli`; use `deepcli deepseek` or `deepcli kimi` to pin a provider/model for native terminal chat.",
                ...
                "Use `deepcli completion zsh|bash|fish` to install shell completion when you prefer command-line workflows over interactive slash commands.",
                "Use `/compiler plan --smoke` before `deepcli install compiler --smoke` when Docker/compiler dependencies may need installation.",
        ...
        CommandHelp {
            name: "/goal",
            listing: "/goal [objective...|show|status|gate|edit|pause|resume|complete|block|clear] [--json] [--output path] [--acceptance-cmd cmd] [--token-budget n]",
            summary: "Create, manage, and gate a Codex-style persistent session goal contract.",
            usage: &[
                "/goal",
                "/goal <objective> [--token-budget n]",
                "/goal start <objective> [--token-budget n]",
                "/goal edit <objective>",
                "/goal pause",
                "/goal resume",
                "/goal complete",
            ...
            examples: &[
                "/goal 完整实现当前项目文档中的全部需求 --token-budget 120000",
                "/goal pause",
                "/goal resume",
                "/goal complete --json",
                "/goal status --json",
            ...
            notes: &["With no existing goal, `/goal` creates a default project-document goal for the active session. Active goals are injected into future agent context and tell the agent not to stop until requirements, acceptance commands, tests, and final verification all pass. `/goal pause`, `/goal resume`, `/goal edit`, `/goal complete`, `/goal block`, and `/goal clear` manage the current session goal; `/goal complete` only succeeds after the readiness gate passes. Use `/goal status` for the stable `deepcli.goal.status.v1` readiness report, and `/goal gate` when a script should exit non-zero until the goal is ready to complete. `/goal show/status/gate` fall back to the latest session with a goal when no active goal is selected; create and lifecycle changes require an active session."],
        },
        CommandHelp {
            name: "/plan",
        ...
        CommandHelp {
            name: "/agent",
            listing: "/agent list|show|resume|logs [--json] [--output path]|spawn <task>",
            ...
            examples: &[
                ...
                "/agent resume 6155c14e --json",
                "/agent logs 6155c14e --json",
                "/agent spawn inspect failing compiler tests",
            ],
            notes: &["Sub-agent depth is bounded by configuration.", "`/agent show|resume|logs` accept a unique id prefix. `resume` executes queued, running, or failed tasks through a real child `AgentRuntime`; failures are persisted on the task with event logs and child session metadata. Use `--json` for the stable `deepcli.agent.inspect.v1` schema and `--output` to write the selected format to a workspace-contained file."],
        },
        CommandHelp {
            name: "/btw",
        ...
        CommandHelp {
            name: "/stop",
            listing: "/stop",
            summary: "Stop the currently running interactive task and keep the session resumable.",
            usage: &["/stop"],
            examples: &["/stop"],
