...
Ready round 顶层 baseline 后续动作应复用 `opportunity_baseline_next_actions(sota_baseline_next_actions(workspace))`，先给 `deepcli benchmark baselines --json`，再给 `baseline-template` 或 `compare`；`round.checklist[]` 必须从这个 inventory-first 队列派生，避免产品循环页直接跳到写本地 baseline artifact。
...
- `recipes sota` 的状态对象应复用同一次 `build_round_report` 的 `next_actions` 和 `opportunities`，避免 recipe 入口重新定义产品机会；`deepcli.recipes.v1` 顶层输出 `recommendedOpportunity`、`opportunityPriorityCounts` 和 `opportunities[]`，普通 topic 返回空数组，文本输出在有机会时展示同样的 recommended opportunity、priority counts、summary、impact、priority、effort 和 next actions。
...
- `opportunity_baseline_next_actions(raw_actions)` 负责把共享 baseline 导航包装成用户可见的 inventory-first 队列；`opportunities` 和 ready `round` 必须通过该包装器暴露 baseline 后续动作，先展示 `deepcli benchmark baselines --json`，再展示原始 `baseline-template` 或 `compare` 动作，并从包装后的队列派生 checklist。
