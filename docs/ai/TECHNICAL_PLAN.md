...
- `recipes sota` 的状态对象应复用同一次 `build_round_report` 的 `next_actions` 和 `opportunities`，避免 recipe 入口重新定义产品机会；`deepcli.recipes.v1` 顶层输出 `opportunities[]`，普通 topic 返回空数组，文本输出在有机会时展示同样的 summary、impact、effort 和 next actions。
