...
- `/fork [session_id|--current] [--dry-run|--no-open] [--json] [--output path]` 复制已持久化会话上下文到新 session id。实现从当前 session、显式 session id 或最近有记录活动的 session 选择源会话，创建新 metadata，复制源 session 目录中除 `metadata.json` 外的持久化文件，给副本标题加 `Fork of ...`，记录 source/fork audit，并输出稳定 `deepcli.session.fork.v1`。默认通过 macOS Terminal 执行 `deepcli resume <new_id>` 打开新终端；`--dry-run|--preview` 只返回 `status=dry_run`、`dryRun=true`、`fork=null`、`plannedFork`、`terminal.wouldOpen`、`contextCopy` 和 next actions，不创建 session、不复制文件、不打开 Terminal；`--no-open` 用于 CI、测试或只需要 fork id 的场景，会真实创建 fork 但跳过 Terminal。JSON 报告包含 `contextCopy`，声明复制模式为 `persisted_session_files`、`hotForkSupported=false`、源会话状态、运行中任务状态和 warning；真实 fork 的 `nextActions` 至少包含 `deepcli resume <new_id>`，dry-run 的 `nextActions` 至少包含可执行的真实 fork 命令；源会话处于 running 时还应提示 `deepcli stop` 以及任务结束后可重新 fork 以包含最终输出。TUI 运行中也允许执行 `/fork --current` 和 `/fork --current --dry-run --json`，但当前正在运行的后台 Agent 任务不宣称可热分叉。
