...
- `/fork [session_id|--current] [--dry-run|--no-open] [--verify] [--json] [--output path]` 复制已持久化会话上下文到新 session id。实现从当前 session、显式 session id 或当前 workspace 最近的可恢复对话上下文选择源会话；无 active session 且无 id 时复用 resume 候选过滤，跳过空会话、tool-only 或诊断型 session，避免默认 fork 到没有可继续上下文的记录；shell 中显式传 `--current` 但没有 active session 时返回带 `deepcli fork`、显式 session id 和 `deepcli sessions --all` 的可执行提示。创建新 metadata 后复制源 session 目录中除 `metadata.json` 外的持久化文件，给副本标题加 `Fork of ...`，记录 source/fork audit，并输出稳定 `deepcli.session.fork.v1`。默认通过 macOS Terminal 执行 `deepcli resume <new_id>` 打开新终端；`--dry-run|--preview` 只返回 `status=dry_run`、`dryRun=true`、`fork=null`、`plannedFork`、`terminal.wouldOpen`、`contextCopy` 和 next actions，不创建 session、不复制文件、不打开 Terminal；当 `--json` 下源会话选择失败时，通过 `CommandExit` 保留同一 schema 的 `status=error` 报告，包含 `source=null`、`fork=null`、`plannedFork=null`、`contextCopy=null`、`error.code`、脱敏后的 `error.message`、可执行 `nextActions` 和 `report`，并先写入合法的 `--output` 再返回非零，避免外部 UI/脚本解析纯文本错误；`--no-open` 用于 CI、测试或只需要 fork id 的场景，会真实创建 fork 但跳过 Terminal；`--verify` 只在真实 fork 后读取源和副本的持久化 session 文件，输出 `verification.status`、`resumeReady`、workspace/provider/model 匹配状态、fork state、resume command，以及 message/tool/test/diff/backup 计数一致性，不执行 provider 或真正启动 resume。JSON 报告包含 `contextCopy`，声明复制模式为 `persisted_session_files`、`hotForkSupported=false`、源会话状态、运行中任务状态和 warning；真实 fork 的 `nextActions` 至少包含 `deepcli resume <new_id>`，dry-run 的 `nextActions` 至少包含可执行的真实 fork 命令；源会话处于 running 时还应提示 `deepcli stop` 以及任务结束后可重新 fork 以包含最终输出。TUI 运行中也允许执行 `/fork --current` 和 `/fork --current --dry-run --json`，但当前正在运行的后台 Agent 任务不宣称可热分叉。
