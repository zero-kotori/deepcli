...
`deepcli benchmark status --json`、provider 前缀下的 `deepcli deepseek benchmark status --json` 和 slash 入口 `/benchmark status --json` 也必须在构造 runtime 前映射为本地只读命令，复用 workspace 输出路径校验，不创建 session、不调用 provider、不执行 shell。
...
`/scorecard`、`/sota` 和 `deepcli scorecard ...` 也属于无 session、本地只读入口；provider 前缀下的 `deepcli deepseek scorecard --json` 应在构造 runtime 前映射到同一命令。该入口用于把“接近 SOTA”拆成命令发现、Agent 工作流、会话续跑、验收交付、安全隐私、Provider/模型、支持诊断和 benchmark 证据等可执行评分项，避免产品循环只靠人工主观判断。`/benchmark`、`/bench` 和 `deepcli benchmark ...` 属于同一类本地入口：无子命令或 scorecard flags 时兼容 `/scorecard`，`presets/run/record/status/summary/list/show` 用于发现推荐 workload、执行、沉淀、评估证据质量、汇总和查看 `.deepcli/benchmarks/*.json` 证据 artifact；`presets` 只读展示可执行 preset，`run` 只能在用户显式提供 preset 或命令时执行本地 shell 并受超时约束，`record` 只能记录声明的命令和本地摘要，`status/summary/list/show` 只读取本地 artifact，不能调用 provider；`status` 输出稳定 `deepcli.benchmark.status.v1`，区分 missing、weak、failing、stale 和 ready，避免 smoke-only artifact 被误判为强 benchmark 证据。
