...
`/opportunities`、`/opportunity` 和 `deepcli opportunities ...` 属于无 session、本地只读产品机会入口；provider 前缀下也应在构造 runtime 前映射到同一命令。实现上只调用 `build_round_report(...).opportunities`，并从这些机会的 `nextActions` 展平生成顶层动作和 `checklist[]`；不得复制 `scorecard_product_opportunities` 的判断逻辑。JSON 使用稳定 `deepcli.opportunities.v1`，包含 source round command、round status、ready、opportunityCount、opportunities、nextActions、checklist 和 report；当当前 round 尚未 ready 且机会为空时，顶层动作回退到 `round.nextActions`，让机会页仍能推进修复。
