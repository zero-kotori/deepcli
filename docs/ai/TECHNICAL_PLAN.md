...
- `/fork [session_id|--current] [--no-open] [--json] [--output path]` 复制已持久化会话上下文到新 session id。实现从当前 session、显式 session id 或最近有记录活动的 session 选择源会话，创建新 metadata，复制源 session 目录中除 `metadata.json` 外的持久化文件，给副本标题加 `Fork of ...`，记录 source/fork audit，并输出稳定 `deepcli.session.fork.v1`。默认通过 macOS Terminal 执行 `deepcli resume <new_id>` 打开新终端；`--no-open` 用于 CI、测试或只需要 fork id 的场景。JSON 报告包含 `contextCopy`，声明复制模式为 `persisted_session_files`、`hotForkSupported=false`、源会话状态、运行中任务状态和 warning；`nextActions` 至少包含 `deepcli resume <new_id>`，源会话处于 running 时还应提示 `deepcli stop` 以及任务结束后可重新 fork 以包含最终输出。TUI 运行中也允许执行 `/fork --current`，但当前正在运行的后台 Agent 任务不宣称可热分叉。
