...
所有指令都应能在 Agent 运行期间安全执行；当前 TUI 运行中至少需要保留本地 `/status`、`/usage`、`/trace`、`/logs`、`/recipes`、`/selftest`、`/preflight`、`/completion`、`/approval`、`/session`、`/terminal`、`/stop`、`/quit` 与 `/btw ask/list/answer/clear`，通过当前 session 文件直接读写或按工具权限策略执行，不依赖正在后台执行的 `AgentRuntime`。by-the-way 小问题应进入旁路队列，不破坏主任务状态。
