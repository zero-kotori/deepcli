...
- `/scorecard [--json] [--output path] [--fail-below n]`，在无 session 的 `CommandContext` 中构建产品能力评分；别名 `/sota` 解析到同一实现；评分维度包括 command discovery、agent workflow、session continuity、verification/delivery、safety/privacy、provider/model ops、support operability 和 benchmark evidence；文本输出保持终端可读，JSON 输出稳定 `deepcli.scorecard.v1`，包含 score、maxScore、percent、tier、categories、gaps、nextActions 和 report；每个 category 保留完整分类级 nextActions；全局 nextActions 在存在 gaps 时只聚合 priority 修复动作、有 gap 分类动作和 SOTA 产品循环动作，没有 gaps 时再展示各分类通用探索动作，避免本轮修复队列被强分类导航淹没；`--output` 通过 workspace path 校验写入当前格式；`--fail-below` 在总分低于阈值时保留完整输出并返回非零，供产品循环和 CI 门禁使用；该命令不创建 session、不调用 provider，运行中 TUI 可直接执行。
...
- `/fork [session_id|--current] [--no-open] [--json] [--output path]` 复制已持久化会话上下文到新 session id。实现从当前 session、显式 session id 或最近有记录活动的 session 选择源会话，创建新 metadata，复制源 session 目录中除 `metadata.json` 外的持久化文件，给副本标题加 `Fork of ...`，记录 source/fork audit，并输出稳定 `deepcli.session.fork.v1`。默认通过 macOS Terminal 执行 `deepcli resume <new_id>` 打开新终端；`--no-open` 用于 CI、测试或只需要 fork id 的场景。当前正在运行的后台 Agent 任务不宣称可热分叉；如果源会话处于 running，应提示先等待完成或 `/stop` 后再 fork。
