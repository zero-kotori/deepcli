...
- TUI 和 message box：确定采用 `ratatui + crossterm`；message box 采用自研输入组件，必须支持 `Shift+Enter` 换行、`Enter` 提交、Left/Right、Home/End、Delete、Backspace、Ctrl-A/Ctrl-E、Ctrl-U/Ctrl-K、真实光标渲染、bracketed paste 粘贴大段文本并插入当前光标位置、slash 命令提示/Tab 补全、running-safe 命令标记、运行中优先展示 running-safe 命令、低信息输入本地追问且避免 `waiting_user` 短回复循环拦截、Agent 运行时旁路提问、running-safe `/terminal` 同目录终端打开、running-safe `/fork --current` 持久化上下文分支，以及运行中 `/session restore-backup --dry-run --json` 只读预览；真实恢复或带 `--output` 的预览写入继续要求等待任务结束或先 `/stop`。
