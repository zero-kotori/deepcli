...
当前本地自评中，`scorecard` 为 80/80，`benchmark status` 为 ready；该结果依赖 `.deepcli/benchmarks/` 下的本地忽略证据 artifact，这些文件不应推送到远程仓库。
