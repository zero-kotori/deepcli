...
- Agent Runtime、ContextManager、ToolRegistry、ToolExecutor、PermissionEngine。
...
- `src/tools.rs` 与 `src/tools/*.rs` 负责工具声明和执行。
- `src/permissions.rs` 负责权限模式、sandbox、风险分级和审批。
- `src/session.rs` 负责会话持久化。
