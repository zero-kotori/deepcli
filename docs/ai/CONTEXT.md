...
本轮继续补齐观测/恢复报告动作清单，让 status、usage、logs、next、session next 和 session diagnose 与 onboarding、自检、health、support、scorecard、round、preflight、交付、benchmark 报告保持同一套可直接渲染的动作队列。
...
./scripts/deepcli next --json | jq '{checklist:.checklist[0:4],nextActions:.nextActions[0:4],quickLinks:.quickLinks[0:4]}'
./scripts/deepcli session diagnose --json | jq '{checklist:.checklist[0:4],recommendedNextActions:.recommendedNextActions[0:4],quickLinks:.quickLinks[0:4]}'
