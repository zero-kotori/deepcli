...
3. 长期 goal、需求澄清 plan、会话 fork
   ...
   - `/plan` 无参数保留查看执行计划；带需求文本时进入模型驱动的只读规划模式，模型会读代码上下文、必要时通过 `ask_user_question` 入队定制问题，并返回具体实现计划；最终计划保存为 session `plan.md`，且必须列出 3-5 个 `Critical Files for Implementation`。
