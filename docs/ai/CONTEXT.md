...
29. benchmark trends 单样本历史不足状态
   ...
   - `nextActions` 会优先提示 `deepcli benchmark run-suite --json --fail-on-command`，文本报告同步显示 `status: insufficient_history`。
