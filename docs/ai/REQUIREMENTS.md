...
- `recipes sota --json` 应在顶层输出非阻塞 `recommendedOpportunity`、`opportunityPriorityCounts` 和 `opportunities[]`，复用当前 `round` 的机会对象，每项包含 id、title、summary、impact、priority、effort、status、nextActions 和 checklist；文本模式有机会时也必须在机会列表前展示推荐机会和优先级计数；普通 recipe 可返回空数组，避免外部 UI 进入产品循环入口后只能看到按钮而看不到机会说明。
