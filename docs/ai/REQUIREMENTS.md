...
- `round --json` ready 状态的外层 baseline 导航也必须 inventory-first：当所有 gates 通过且后续需要 current capture、手工 competitor template 或 compare 时，顶层 `nextActions` 必须先包含只读 `deepcli benchmark baselines --json`，再包含 `baseline-template` 或 `compare` 动作，并从同一队列派生 `checklist[]`，避免产品循环页直接跳到写 `.deepcli/baselines/*` artifact。
