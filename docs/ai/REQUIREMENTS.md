...
- `/fork [session_id|--current] [--dry-run|--no-open] [--json] [--output path]`：复制已持久化会话上下文到新 session id；默认打开新 macOS Terminal 执行 `deepcli resume <new_id>`；`--dry-run` 只预览源会话、复制模式、计划标题、是否默认打开终端和 next actions，不创建 session、不复制文件、不打开 Terminal；`--no-open` 用于真实创建 fork 但跳过 Terminal；JSON 应包含 `dryRun`、`contextCopy`、`hotForkSupported=false`、源会话状态和 next actions，明确不热复制 running Agent 任务。
