...
- 任务型工作流、产品评分和 benchmark 证据入口也应作为本地 one-shot 支持，例如 `deepcli recipes`、`deepcli recipes release --json`、`deepcli playbook support`、`deepcli workflow debug`、`deepcli scorecard --json`、`deepcli benchmark --fail-below 85`、`deepcli benchmark presets --json`、`deepcli benchmark run --preset cargo-test --json --fail-on-command`、`deepcli benchmark record --json --suite product --case scorecard`、`deepcli benchmark status --json`、`deepcli benchmark gate --json`、`deepcli benchmark list --json`、`deepcli benchmark summary --json` 以及 provider 前缀下的 `deepcli deepseek recipes release --json`、`deepcli deepseek scorecard --json`；这些入口只展示可复制命令清单、产品能力覆盖、benchmark 差距或本地 artifact 证据，不应创建 session 或调用 provider。
...
- 支持 Agent 运行期间执行本地 `/status`、`/usage`、`/trace`、`/logs`、`/recipes`、`/scorecard`、`/benchmark`、`/selftest`、`/preflight`、`/completion`、`/approval`、`/session`、`/terminal`、`/stop`、`/quit` 与 by-the-way 问题记录、查看、回答和清理；`/stop` 应中断当前任务并保留可恢复会话，`/quit` 在运行中应先停止任务再退出。
