...
- Agent Runtime：上下文准备、Provider 调用、工具调用循环、会话观测、计划与结果输出。
- 工具系统：文件、shell、Git、测试、环境、web、terminal、prompt、skill、子 Agent，统一通过工具声明、参数校验和权限层。
- 权限与安全：工作区授权、ignore/隐私过滤、sandbox、高风险操作识别、审批队列、凭据脱敏和隐私扫描。
...
角色包括 CLI 用户、Agent、auto-reviewer 和子 Agent。权限模式分为只读、写入、完全控制和 sandbox；默认行为应保守，所有写入、shell、Git、网络、Docker、终端和 setup 动作必须经过工具系统与权限层。
