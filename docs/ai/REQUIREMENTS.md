...
- `/scorecard [--json] [--output path] [--fail-below n]` 与别名 `/sota`：展示产品能力覆盖和 SOTA 差距，按命令发现、Agent 工作流、会话续跑、验收交付、安全隐私、Provider/模型、支持诊断和 benchmark 证据给出分数、tier、gaps 和 next actions；不得创建 session 或调用 provider；`--json` 输出稳定 `deepcli.scorecard.v1` schema，包含 categories、score、percent、tier、gaps、nextActions 和 report；`--output` 必须限制在 workspace 内；`--fail-below` 在分数低于阈值时保留当前文本/JSON 报告并返回非零，供产品循环、CI 或发布脚本把“接近 SOTA”变成可执行门禁。`/benchmark [record|list|show|scorecard]` 与别名 `/bench` 保留无子命令和 scorecard flags 的兼容行为；`record` 应写入 `.deepcli/benchmarks/*.json` 的稳定 `deepcli.benchmark.record.v1` artifact，记录 suite、case、notes、声明的 benchmark command、Git 状态摘要和 scorecard 摘要，但不得隐式执行 shell；`list` 输出稳定 `deepcli.benchmark.list.v1`；`show latest|name` 展示单个 artifact；所有 `--output` 必须限制在 workspace 内。
