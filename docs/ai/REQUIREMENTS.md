...
- `/opportunities [--json] [--output path]` 与别名 `/opportunity`：展示当前非阻塞产品机会，复用同一次 `round` 的 `opportunities[]` 作为唯一来源，不重新定义机会对象；不得创建 session、调用 provider 或执行 shell；`--json` 输出稳定 `deepcli.opportunities.v1` schema，包含 round status、ready、source、opportunityCount、opportunities、nextActions、checklist 和 report；顶层 `nextActions` 从机会动作展平去重，`checklist[]` 从这些动作派生 step/label/command；当当前 round 未 ready 且没有非阻塞机会时，`nextActions` 应回退为 round 修复队列，避免机会页成为无下一步的空报告。
