...
- `recipes sota --json` 应在顶层输出非阻塞 `opportunities[]`，复用当前 `round` 的机会对象，每项包含 id、title、summary、impact、priority、effort、status、nextActions 和 checklist；普通 recipe 可返回空数组，避免外部 UI 进入产品循环入口后只能看到按钮而看不到机会说明。
...
- `/opportunities [--json] [--output path]` 与别名 `/opportunity`：展示当前非阻塞产品机会，复用同一次 `round` 的 `opportunities[]` 作为唯一来源，不重新定义机会对象；不得创建 session、调用 provider 或执行 shell；`--json` 输出稳定 `deepcli.opportunities.v1` schema，包含 round status、ready、source、opportunityCount、opportunities、nextActions、checklist 和 report；每个 opportunity 必须包含 `impact`、`priority` 和 `effort`，让外部 UI 能同时展示收益、优先级和执行成本；顶层 `nextActions` 从机会动作展平去重，`checklist[]` 从这些动作派生 step/label/command；ready 的 `scorecard`、`round` 和 `recipes sota` 顶层 `nextActions` 必须包含 `deepcli opportunities --json`，让外部 UI 和用户能直接打开机会页而不必解析完整报告；benchmark evidence ready 但 freshness 为 aging 时，必须把刷新 benchmark evidence 作为非阻塞高优先级低成本机会输出，且不得改变 ready/gate 语义；baseline 机会必须先给出 `deepcli benchmark baselines --json` 只读 inventory 动作，再给生成 current/competitor baseline 或 compare 的动作，避免机会页首个按钮直接写本地 evidence artifact；当当前 round 未 ready 且没有非阻塞机会时，`nextActions` 应回退为 round 修复队列，避免机会页成为无下一步的空报告。
