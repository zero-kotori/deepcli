...
- 会执行本地 shell、恢复文件或维护 artifact 的 `/round --run-benchmark`、`/benchmark run*|record|baseline-template|clean`、`/session restore-backup` 真实恢复、`/session restore-backup --dry-run --output` 和完整 `/preflight` 需要等当前 Agent 任务结束或先 `/stop`。
