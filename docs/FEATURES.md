...
- `deepcli terminal [--dry-run|--no-open] [--json]`：打开当前 workspace 的新终端，或输出可脚本验收的 `deepcli.terminal.v1` 预览；JSON 包含可直接复制的 `workspaceCommand`，并且 dry-run、失败和真实打开成功时的 `nextActions` 都只输出可执行的 `cd <workspace>` 或 `deepcli ...` 命令。
...
- 会执行本地 shell、修改 session metadata、导出/写入 artifact、删除会话、Git 写操作或恢复文件的 `/round --run-benchmark`、`/benchmark run*|record|baseline-template|clean`、`/git ... --output`、`/git create-branch`、`/git commit`、`/session rename`、`/session export`、`/session prune-empty --force`、`/session ... --output`、`/session restore-backup` 真实恢复和完整 `/preflight` 需要等当前 Agent 任务结束或先 `/stop`。
