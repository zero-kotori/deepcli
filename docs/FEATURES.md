...
Status、模型、超时、日志、Prompt、Skill 和 Agent 查看类 JSON 的结构化 `nextActions` 都是可直接执行的 `deepcli ...` 命令；`status.session.nextActions` 会根据会话信号给出 `deepcli next/session diagnose` 或 `deepcli usage/trace`，有具体条目时会优先输出具体 prompt 名称、skill 名称或 agent 短 id，说明性上下文留在 `report` 或条目字段。
