...
`recipes sota --json` 顶层还会输出非阻塞 `recommendedOpportunity`、`opportunityPriorityCounts`、`opportunityEffortCounts` 和 `opportunities[]`，复用当前 `round` 的机会对象，供 TUI、外部产品循环页或脚本在同一个入口展示机会说明、影响、优先级、成本、状态、动作清单和成本分布；文本模式会在机会列表前展示推荐机会、优先级计数与成本计数。
