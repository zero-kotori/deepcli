...
- `deepcli benchmark presets|run|record|summary|list|show [--json]`：发现推荐 workload、执行、记录、汇总、列出和查看本地 benchmark 证据 artifact。
