...
- `deepcli git status|diff|branch|message --json`：用稳定 `deepcli.git.inspect.v1` 输出只读 Git 检查结果、原始 stdout/stderr、report 和可执行 next actions；`diff` 支持 `--staged|--cached`，未知只读参数会被拒绝，避免脚本把空输出误判为成功。
