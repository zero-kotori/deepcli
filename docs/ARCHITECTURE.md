...
scripts/deepcli (wrapper)         顶层命令/别名 → slash，自动构建二进制、注入 -C/--yes/凭据
  └─ src/cli.rs                   参数解析、provider/模式归一化、one-shot 路由、交互入口选择
       ...
       └─ src/ui.rs + src/ui/*    原生终端 UI：native terminal 聊天入口、bracketed paste 输入编辑、输出控制序列清洗、低噪声流式对话、失败/审批状态、plan 采访文本选项和文本 resume picker
```
...
- `ToolExecution.success` 是 Provider tool result、审计生命周期和 UI 错误状态的共同结果来源；native terminal 不展示 Provider 请求指标或成功工具进度，只投影失败、审批等需要用户关注的状态。
